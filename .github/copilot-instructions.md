...
並且於最後替整個建議內容總結一長度不超過 50 個中文字的摘要；這是一份額外的內容，其不影響原本的建議內容，這份內容的主要目的是給出一個結果，如下：

- 完美通過：沒有任何程式的 BUG，且符合程式碼品質的要求。
- 通過：沒有任何程式的 BUG，但程式碼品質存在提升空間。
- 不通過：存在程式碼 BUG；不管 BUG 是否嚴重，只要存在 BUG 就不行。
